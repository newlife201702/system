<template>
    <el-dialog v-model=dialogVisible :title="currentAction === 'edit' ? '编辑任务' : '添加任务'" width=840>
...
let ruleForm = reactive<ReqPlanTaskAddFormData>({
    about: '', //说明
    code: '', //编号
...
const goSave = async () => {
    const result: any = await api.planTask.add({ ...ruleForm, personRelaList: getPersonRelaList() })
...
const goEdit = async () => {
    const result: any = await api.planTask.update({ id: id.value, model: { ...ruleForm, personRelaList: getPersonRelaList() } })
    if (result.returnCode === 0 && result.resultCode === 0) {
...
const openDialog = (row: any, actionType: string): void => {
    ...
    nextTick(() => {
        ...
        if (actionType === 'edit') {
            ...
            if(row.personRelaList.length>0)
            {
