          ...
          >
            <el-card class="box-card">
              ...
              <div>
                ...
                <span>
                  <el-tag>{{ item.postLevelName }}</el-tag>
                  <el-tag>{{ item.dataSecurityLevelName }}</el-tag></span
                >
                <div>电话：{{ item.phone }}</div>
              </div>
              <span v-if="isCheck(item)"
                ><el-icon>
                  <CircleCheck /> </el-icon
              ></span>
            </el-card>
...
const submitData = async () => {
  const paramsData: any = {
    roleList: [{id:currentRoleItem.value.id}],
    userList: currentRowList.value.map((item:any)=>{return {
        "id": item.id,
        "openId":item.openId,
        "name": item.name,
        "code":item.code,
  ...
  const result = await api.role.batchAdd(paramsData)
...
const openDialog = (row: any): void => {
  currentRoleItem.value = row
  currentRowList.value = row?.users
  console.log(row)
  limit.value = 12
  query.value = formInline
  loadList()
